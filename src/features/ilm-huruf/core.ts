...
function generateDailyTips(
  ...
  // Band-based tip
  if (band === 'High') tips.push('Strong flow—schedule key tasks or new starts.');
  else if (band === 'Moderate') tips.push('Solid day—focus and follow through.');
  else tips.push('Gentle day—plan, review, avoid overload.');
  
  // Element balance
  const elementTips = {
    Fire: 'Cool balance—calm speech, charity, water.',
    Water: 'Activate—light exercise, sunlight.',
    Air: 'Ground—routine, nature walk, single-tasking.',
    Earth: 'Lighten—creativity, flexibility, short trip.'
  };
  tips.push(elementTips[element]);
  
  // Planet flavor
  const planetTips: Record<Planet, string> = {
    Sun: 'Lead briefly, avoid ego traps.',
    Moon: 'Care for emotions, simplify.',
    Mars: 'Take action, avoid haste.',
    Mercury: 'Communicate and learn.',
    Jupiter: 'Think big, seek blessing.',
    Venus: 'Beautify, reconcile, rest.',
    Saturn: 'Plan, structure, be patient.'
  };
  tips.push(planetTips[planet]);
